...
#每个元素是一个解
		...
		{
			my $tmm=$_;
	#		for(0 .. scalar(@{$members[$tmm]})-1)
	#		{
	#			my $t2mm=$_;	
#print "-$tmm--$t2mm---$members[$tmm][$t2mm]\n";
## @xsjset
#print "$xsjset[$members[$tmm][$t2mm]][0]\n";
...
#		}
#		if(scalar(@{$members[$tmm]}) == scalar(@{$xsj[$tmm]}))
		
#$jres=$a->{交集} ro 0
#	$b->{交集} ro 0
			
			#@unblhash=unbl(\@{$rulabc[$abcn]});
# $unblhash= $a->{$线索组id}->{解数组}
# $unblhash= $b->{$线索组id2}->{解数组}
# $unblhash= $c->{$线索组id}->{解数组}
			#$ujres=un_res($bbl)			
#$ujres= $a->{$n+m个几和外的解数组}
#$ujres= $b->{$n+m个几和外的解数组}
#$ujres= $c->{$n+m个几和外的解数组}
...
##!/usr/bin/perl
#use Smart::Comments;
#@a=("南美","的","青蛙","是","一种","有些","发","淡淡","绿色","的","动物");
#@b=("\$a","是","\$b","绿色","\$d");
#@b=("\$a","是","\$b","\$c","一种");
#match_2_ex(\@a,\@b);
sub match_2_ex
...
#			{
#		
#                        $tmpbigo=Set::Scalar->new(@tmpv);
#				$firstv++;
#			next;}
#			elsif($firstv==2)
#			{		
#				$firstv++;
#                      my  $tmpbig2=Set::Scalar->new(@tmpv);
#                         my $c = $tmpbigo->cartesian_product($tmpbig2);
#                        $tmpt=$c->copy;
#                     my   @members  = $tmpt->members;
#                        $tmpt=();
#                for(0.. scalar(@members)-1)
#                {
#                        my $t9=$_;
...
#			else
#			{
#			 my  $tmp2=Set::Scalar->new(@tmpv);
#			my  $tmp3=Set::Scalar->new(@{$ttt});
#			 my $c = $tmp3->cartesian_product($tmp2);
#		my		@members  = $c->members;
#				for(0.. scalar(@members)-1)
#		                {
#	                        my $t9=$_;
#	                        $str=join(" ",@{$members[$t9]});
...
#        while(my ($k1,$v1)=each(%{$hash_un_v}))
#  	{
#                                if($v1>=$hash_un_v->{$k})
#                                {
#                                        print "发现唯一变量解 \$k =$k1,$v1\n";
#                            	 }
#       }
